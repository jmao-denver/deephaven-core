...
# every method that depends on symbols defined via _defineSymbols() should be decorated with
...
def convertToJavaHashMap(input1, input2=None):
    ...
    elif (isinstance(input1, list) or isinstance(input1, tuple) or isinstance(input1, numpy.ndarray) or _isJavaArray(
            input1)) and \
            (isinstance(input2, list) or isinstance(input2, tuple) or isinstance(input2, numpy.ndarray) or _isJavaArray(
                input2)):
    ...
    if javaType is None:
        ...
        try:
            ...
            return _failedToConvertArray(data, name, data[0], convertUnknownToString,
                                         ...
                                         "of list or tuple, but conversion attempts failed with error: {}.".format(name,
                                                                                                                   e))
...
def _tupleToColDef(t):
    ...
    if not _isStr(component_type_name):
        raise Exception(
            'Element at index 2 (' + component_type_name + ') for component type name is not of string type')
...
def _custom_charCol(name, *data):
    ...
    if len(data) == 1:
        if _isJavaType(data[0]):
            ...
            if data[0].dtype == numpy.uint16:
            ...
            elif data[0].dtype in [numpy.int8, numpy.uint8, numpy.int16, numpy.int32, numpy.uint32, numpy.int64,
                                   numpy.uint64]:
                # not entirely sure that this is necessary
                return _java_type_.charCol(name, jpy.array('char', data[0].astype(numpy.uint16)))
            elif data[0].dtype == numpy.dtype('U1') and data[0].dtype.name in ['unicode32', 'str32', 'string32',
                                                                               'bytes32']:
                ...
                # do our best
                return _java_type_.charCol(name, jpy.array('char', numpy.array([makeElementChar(el) for el in data[0]],
                                                                               dtype=numpy.uint16)))
            else:
